...
cleaned_text_unit_v1
  ...
  -> P4-full RAGEngine + PromptBuilder + LocalLLMGenerator + DataCapture
  -> answer
...
注意：
- 本脚本会调用本地 LLM，通过 local_llm.py 生成 answer。
- 如果只想测试到 prompt/capture，请继续使用 test_full_parent_child_rag_capture_pipeline.py。
- 本脚本定位为 RAG QA v1.0 的最终单机回归测试。
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Full parent-child hybrid RAG QA pipeline test: P0 -> P1 -> P2 -> P3 -> P4-full"
    )
    ...
    parser.add_argument("--llm-model", default=LLM_MODEL_NAME or r"D:\models\huggingface\llm\Qwen2.5-1.5B-Instruct")
...
def run_p1_p2_p3_p4(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    print("\n========== P4-full RAGEngine + PromptBuilder + LocalLLM + DataCapture ==========")
    ...
    engine = ParentChildRAGEngine(
        ...
        pipeline_name="parent_child_hybrid_rag_qa_full_pipeline",
    ...
    print("\n----- Answer -----")
    print(engine_result.get("answer"))
    ...
    assert engine_result.get("answer") and str(engine_result.get("answer")).strip(), "P4-full should generate non-empty answer"
    assert engine_result["run_record"].get("answer") == engine_result.get("answer"), "run_record should save answer"
    assert engine_result["run_record"].get("model_name"), "run_record should save model_name"
    assert engine_result["run_record"].get("generation_params"), "run_record should save generation_params"
...
def main() -> None:
    ...
    print("========== Full Parent-Child RAG QA Pipeline ==========")
    ...
    final_summary = {
        ...
        "answer_preview": (downstream["engine_result"].get("answer") or "")[:500],
        "model_name": downstream["engine_result"].get("model_name"),
        "generation_params": downstream["engine_result"].get("generation_params"),

"""
scripts/eval_full_milvus.py
===========================

运行基于 Milvus Lite 的完整 RAG Pipeline 评估。
...
def main() -> None:
    print("=" * 80)
    print("[eval_full_milvus] 初始化 TextEmbedder")
    ...
    print("=" * 80)
    print("[eval_full_milvus] 初始化 MilvusRetriever")
    ...
    if USE_RERANKER:
        print("=" * 80)
        print("[eval_full_milvus] 初始化 TextReranker")
    ...
    print("=" * 80)
    print("[eval_full_milvus] 初始化 LocalLLMGenerator")
    ...
    eval_report_path = PROCESSED_DATA_DIR / "eval_full_milvus_report.json"

    eval_top_k = RETRIEVAL_TOP_K if USE_RERANKER else FINAL_TOP_K

    print("=" * 80)
    print("[eval_full_milvus] 开始运行完整 RAG 评估")
